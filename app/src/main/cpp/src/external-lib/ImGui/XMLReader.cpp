...
inline std::string& XMLReader::ltrim(std::string& s, const char* t) {
    s.erase(0, s.find_first_not_of(t));
    return s;
}

// trim from both ends of string (right then left)
inline std::string& XMLReader::trim(std::string& s, const char* t) {
    return ltrim(rtrim(s, t), t);
}

std::vector<std::string> XMLReader::tokenize(const std::string& line, const std::string& delimiters) {
...
int XMLReader::parseFlags(std::string& flagstr) {
...
XMLNode* XMLReader::stringToNode(std::string& str) {
    std::regex tagname = std::regex(R"(<\/?[^\s>\/]+)");
    std::smatch m;
...
XMLTree* XMLReader::traverse(std::vector<std::string>& tags) {
...
XMLTree* XMLReader::readFromString(std::string xml_content) {
    std::istringstream file(xml_content);
...
const std::unordered_map<std::string, ImGuiEnum> XMLReader::tagnames = {{"begin", ImGuiEnum::BEGIN},
...
const std::unordered_map<std::string, int> XMLReader::flagnames = {
    ...
    {"ImGuiWindowFlags_AutoCollapsed", ImGuiWindowFlags_AutoCollapsed},
